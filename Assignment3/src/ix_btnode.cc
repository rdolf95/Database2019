...
//************************************************
// If this method is executed, redestribution is checked valid.
RC IX_BTNode::DistributeNode(IX_BTNode * aLeftNode){
    ...
    RC rc = aLeftNode->InsertNode(k, rid);
    if (rc != 0) return rc;

    void * keytodel;
    GetKey(0, keytodel);
    rc = DeleteNode(keytodel, -1);
    if (rc == -1) return rc;
