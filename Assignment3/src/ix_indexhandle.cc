...
RC IX_IndexHandle::InsertEntry(void *pData, const RID &rid, int detail) {
    ...
    if(result == -1){
        IX_BTNode* leftNode = FetchNode(node->GetLeft());
        if (leftNode != NULL){
            IX_BTNode* parentNode = path[level -1];
            
            void * leftLargest = leftNode->LargestKey();
            // Find left path.
            FindLeaf(leftLargest);
            IX_BTNode* leftParent = path[level -1];
	        int posAtLeftParent = pathP[level -1];
        
            if ((leftParent == parentNode) 
                && (leftNode->GetKeysNum() < leftNode->GetOrder())){
                // Able to Redistribute                
                // Left is not full.
		        
                // change path to inserting node
    		    FindLeaf(pData);

                //Distribute
                result = node->DistributeNode(leftNode);

                // update old key - keep same addr
                leftParent->DeleteNode(NULL, posAtLeftParent);
                leftParent->InsertNode((const void*) leftNode->LargestKey(),
                leftNode->GetNodeRID());

                if (result == 0){
                    // Insert data to node.
                    if (node->CompareKey(pData, leftNode->LargestKey()) >= 0) {
                        node->InsertNode(failedKey, failedRid);
                    } else { // <
                        leftNode->InsertNode(failedKey, failedRid);
                    }
